...
typedef struct {
  ...
  uint64 skipcount;  	 /* skip as soon as leftcount=0 (64 bits) */
  uint64 bytecount;	 /* total i/o byte count (64 bits) */
} file_info_t;

static file_info_t *file_info[MY_FOPEN_MAX];
...
static file_info_t *getInfo(int fd)
{
  ...
  if (! initialized) {
    int i;
    for (i=0;i<MY_FOPEN_MAX;i++) {
      file_info[i] = 0;
    }
    initialized = 1;
  }
  else if (fd >= MY_FOPEN_MAX) {
...
int data_read(int fd,void* buffer,int bufsize)
{
  ...
  if (infd == 0) {
    ...
    if (infd->link == 0) {
      ...
      char version[99];
      char str[80];
      char *s = fgets(str,80,infd->fp);
      if (s == 0 && feof(infd->fp)) {
        return DATAIO_EOF;
      }
      else if (s == 0) {
        fprintf(stderr,"dataIO.data_read: read error from sieve header\n");
#ifdef USE_ALARM
        esend(ALARM_ERROR,"dataIO.data_read: sieve header read error\n");
#endif
        return DATAIO_ERROR;
      }
      ret = sscanf(str,"DATA SIEVE version %s\n",version);
      if (ret == 0) {
    ...
    while (infd->leftcount == 0) {
      ...
      if (infd->skipcount == 0) {
        ...
        char filename[999];
        data_flush(fd);
        s = fgets(str,80,infd->fp);
        if (s == 0 && feof(infd->fp)) {
          return DATAIO_EOF;
        }
        else if (s == 0) {
          fprintf(stderr,"dataIO.data_read: sieve data read error\n");
#ifdef USE_ALARM
          esend(ALARM_ERROR,"dataIO.data_read: sieve data read error.\n");
#endif
          return DATAIO_ERROR;
        }
        ret = sscanf(str,"input=\"%s\"\n",filename);
        if (ret == 1 && (fpin = fopen(strtok(filename,"\""),"r"))) {
          s = fgets(str,80,fpin);
          if (s == 0) {
            fprintf(stderr,"dataIO.data_read: sieve header read error\n");
#ifdef USE_ALARM
            esend(ALARM_ERROR,"dataIO.data_read: sieve header read error.\n");
#endif
            return DATAIO_ERROR;
        ...
        data_ff(infd->link,infd->skipcount);
      }
      ...
      ret = sscanf(str,"%lld,%lld\n",&infd->leftcount,&infd->skipcount);
      if (ret == 1) {
        infd->skipcount = 0;
      }
      else if (ret != 2)  {
...
int data_ff(int fd,off_t skipbytes)
{
  ...
  if (infd && infd->sieve) {
    if (infd->writing) {
      ...
      while (skipbytes && infd->leftcount < skipbytes) {
        int ret;
        if (infd->leftcount > 0) {
          ret = data_ff(infd->link,infd->leftcount);
          if (ret != DATAIO_OK) {
            return ret;
          }
      ...
      return data_ff(infd->link,skipbytes);
    }
    return DATAIO_OK;
  }
